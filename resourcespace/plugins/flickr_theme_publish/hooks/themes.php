...
function HookFlickr_theme_publishThemesCollectiontoolcompact1($collection, $count_result)
    ...
    if ($count_result>0) # Don't show the option if the theme is empty.
        ...
        ?>
        <option value="<?php echo $theme["ref"]?>|0|0|<?php echo $baseurl_short?>plugins/flickr_theme_publish/pages/sync.php?theme=<?php echo $collection?>|main|false">&gt;&nbsp;<?php echo $lang["publish_to_flickr"] ?>...<?php if ($unpublished>0) { echo " <strong>(" . ($unpublished==1 ? $lang["unpublished-1"] : str_replace("%number", $unpublished, $lang["unpublished-2"])) . ")</strong>"; } ?></option>
        <?php
        }
    }
?>
